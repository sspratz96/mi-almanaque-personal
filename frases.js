/* ============================================================
   FRASES INSPIRADORAS
   ------------------------------------------------------------
   Este archivo alimenta la "frase del día" de la app.
   Puedes agregar TODAS las que quieras: el index.html no crece.

   CÓMO AGREGAR UNA FRASE
   Copia una línea nueva dentro de los corchetes, con el formato:

       ["El texto de la frase.", "Autor"],

   - Usa comillas dobles.
   - Si no sabes el autor, pon "Anónimo".
   - Si tu frase lleva comillas dobles adentro, escríbelas así: \"
   - No borres la coma del final de cada línea.

   Después de editar, guarda el archivo y sube frases.js al repo
   junto con index.html. La app elige una distinta cada día y el
   botón ↻ muestra otra al azar.
   ============================================================ */

window.FRASES = [
  ["El mejor momento para plantar un árbol fue hace veinte años; el segundo mejor es hoy.", "Proverbio chino"],
  ["No cuentes los días, haz que los días cuenten.", "Muhammad Ali"],
  ["Lo que haces cada día importa más que lo que haces de vez en cuando.", "Gretchen Rubin"],
  ["Cae siete veces, levántate ocho.", "Proverbio japonés"],
  ["La disciplina es elegir entre lo que quieres ahora y lo que quieres más.", "Anónimo"],
  ["No es la montaña lo que conquistas, sino a ti mismo.", "Edmund Hillary"],
  ["Un viaje de mil kilómetros comienza con un solo paso.", "Lao Tsé"],
  ["Hazlo con miedo, pero hazlo.", "Anónimo"],
  ["La constancia vence lo que la dicha no alcanza.", "Simón Bolívar"],
  ["Quien tiene un porqué para vivir puede soportar casi cualquier cómo.", "Friedrich Nietzsche"],
  ["El que se levanta temprano encuentra la mañana entera por delante.", "Anónimo"],
  ["Pequeños progresos siguen siendo progresos.", "Anónimo"],
  ["No dejes para mañana lo que puedas disfrutar hoy.", "Anónimo"],
  ["La calma es también una forma de avanzar.", "Anónimo"],
  ["Sé el cambio que quieres ver en el mundo.", "Mahatma Gandhi"],
  ["El único modo de hacer un gran trabajo es amar lo que haces.", "Steve Jobs"],
  ["Nuestra mayor gloria no está en no caer nunca, sino en levantarnos cada vez que caemos.", "Confucio"],
  ["La suerte es lo que sucede cuando la preparación se encuentra con la oportunidad.", "Séneca"],
  ["Vive como si fueras a morir mañana; aprende como si fueras a vivir siempre.", "Mahatma Gandhi"],
  ["La felicidad no es algo hecho. Viene de tus propias acciones.", "Dalái Lama"],
  ["Empieza donde estás, usa lo que tienes, haz lo que puedas.", "Arthur Ashe"],
  ["El futuro pertenece a quienes creen en la belleza de sus sueños.", "Eleanor Roosevelt"],
  ["Todo parece imposible hasta que se hace.", "Nelson Mandela"],
  ["No hay atajos para ningún lugar que valga la pena.", "Beverly Sills"],
  ["Haz de cada día tu obra maestra.", "John Wooden"],
  ["La paciencia es amarga, pero su fruto es dulce.", "Aristóteles"],
  ["Un poco cada día lleva lejos.", "Anónimo"],
  ["Respira. Estás exactamente donde necesitas empezar.", "Anónimo"],
  ["El descanso también es parte del trabajo.", "Anónimo"],
  ["Lo simple, bien hecho, es suficiente.", "Anónimo"],
  ["No compares tu capítulo uno con el capítulo veinte de otra persona.", "Anónimo"],
  ["Cada día es una página en blanco; escríbela bien.", "Anónimo"],
  ["Lo que riegas, florece.", "Anónimo"],
  ["Hoy es un buen día para intentarlo de nuevo.", "Anónimo"],
  ["La gratitud convierte lo que tenemos en suficiente.", "Anónimo"],
  ["El esfuerzo de hoy es la tranquilidad de mañana.", "Anónimo"],

  // ↓↓↓ Agrega tus frases nuevas desde aquí ↓↓↓
  ["One way to increase the probability that things will unfold for you properly is to not lie.", "Jordan Peterson"],
  ["Figure out what you want. Block out the noise. Then do so much work that it would be unreasonable that you fail.", "Alex Hormozi"],
  ["Then I heard the voice of the Lord saying, 'Whom shall I send? And who will go for us?' And I said, 'Here am I. Send me!", "Isaiah 6:8"],
  ["Oh Heavenly Father, bear witness as I show you the strength of your creation.", "Anonimous"],
  ["If you have a problem with me, text me. And if you don't have my number, you don't know me well enough to have a problem with me.", "Tom Holland"],
  ["A harmless man is not a good man. A good man is a very, very dangerous man who has that under voluntary control.", "Jordan Peterson"],
  ["A society grows great when old men plant trees in whose shade they know they shall never sit in.", "Greek Proverb"],
  ["One must imagine Sisyphus happy.", "Albert Camus"], 
  ["Even if it’s not your fault, it’s your responsibility.", "Terry Pratchett"],
  ["When you know that you can be the one that you should be, you are living in Hell.", "Sathiya Sam"],
  ["Sometimes you sing lead, and sometimes you're on tambourine. And if you're on tambourine, play it right. Play it with a smile. Because nobody wants to see a mad tambourine player.", "Chris Rock"],
  ["People only get to know you as much as they know themselves.", "Dylan Banks"],
  ["The man who can walk away from what he desires for the sake of what he must become is a man no one can own.", "Epictetus"],
  ["If you are so sure, then burn the ships.", "Hernan Cortes"], 
  ["Trying to make things idiot proof, but they keep making better idiots.", "Anonimous"], 
  ["How do you spell 'love'? T-I-M-E.", "Michael Smoak"],
  ["I think a man only needs one thing in life. He just needs someone to love. If you can't give him that, then give him something to hope for. And if you can't give him that, just give him something to do.", "Scott Campbell as James Liddle, in The Flight of the Phoenix"],
  ["Give a man a purpose and the ability to achieve it, and he will crawl over broken glass with a smile.", "Adam Lane Smith"],
  ["'I can't see a way through', said the boy.'Can you see your next step?' said the horse.'Yes.''Just take that.'", "Charlie Mackesy on The Boy, the Mole, the Fox and the Horse"],
  ["'Your loss', sweetheart, you lied to your people so they would hate me, I lied to my people so they would like you.","Anonymous"], 
];
